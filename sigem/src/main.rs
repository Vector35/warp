...
fn data_from_view(view: &BinaryView) -> signaturebuild::Data {
    let mut data = signaturebuild::Data::default();
    ...
    let functions = view
        ...
        .filter_map(|f| {
            ...
            warp_binja::cache::cached_function(&f, &llil)
        })
        .collect::<Vec<_>>();
...
fn data_from_archive<R: Read>(mut archive: Archive<R>) -> Option<signaturebuild::Data> {
    let mut data = signaturebuild::Data::default();
    ...
    data.functions = entry_data
        .iter()
        .flat_map(|d| d.functions.to_owned())
        .collect();
    data.types = entry_data.iter().flat_map(|d| d.types.to_owned()).collect();

    Some(data)
}

// TODO: Pass settings.
fn data_from_file(path: &Path) -> Option<signaturebuild::Data> {
