...
use symbolbuild::{Symbol, SymbolClass, SymbolModifier};
use typebuild::prelude::*;
use typebuild::r#type::class::array::ArrayModifiers;
use typebuild::r#type::class::function::{Location, RegisterLocation};
use typebuild::r#type::class::pointer::PointerAddressing;
use typebuild::r#type::class::structure::StructureMemberModifiers;
use typebuild::Build;
...
pub fn from_bn_symbol(raw_symbol: &BNSymbol) -> Symbol {
    ...
    match raw_symbol.sym_type() {
        ...
        BNSymbolType::External => Symbol::new(
            ...
            // TODO: Data?
            SymbolClass::Data,
            vec![SymbolModifier::Exported],
        ),
        BNSymbolType::ImportedData => Symbol::new(
            symbol_name,
            SymbolClass::Data,
            vec![SymbolModifier::External],
        ),
        BNSymbolType::LibraryFunction | BNSymbolType::Function => {
            Symbol::new(symbol_name, SymbolClass::Function, vec![])
        }
...
pub fn to_bn_symbol_at_address(view: &BinaryView, symbol: &Symbol, addr: u64) -> BNRef<BNSymbol> {
    let is_external = symbol.modifiers.contains(&SymbolModifier::External);
    let _is_exported = symbol.modifiers.contains(&SymbolModifier::Exported);
...
fn from_bn_type_internal(
    ...
    Type {
        ...
        // TODO: we _do_ want this for lumina (this is the only way we can update type is if we fill this out)
        ancestors: vec![],
    }
}
...
mod tests {
    ...
    fn type_conversion() {
        ...
        // TODO: This will fail for some odd reason, im thinking the other test runs and stops the test file
        // TODO: From being able to see the object file.
        assert_eq!(bv.types().len(), 23);
        let converted_types: Vec<_> = bv
            ...
            .collect();
        assert_eq!(converted_types.len(), 23);
        dbg!(converted_types);
    }
}
